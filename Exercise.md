...
Arrange — Set things up (create the object)
Act — Do the thing you're testing (call the method)
...
MethodWhat it checksassertEquals(expected, actual)Are these two values the same?assertNotEquals(a, b)Are these two values different?assertTrue(condition)Is this true?assertFalse(condition)Is this false?assertNull(object)Is this null?assertNotNull(object)Is this NOT null?
...
javapublic class Circle extends Shape {
...
javaassertEquals(78.54, myCircle.getArea(), 0.01);
...
Key assertions:
javaassertEquals(expected, actual);        // check equal
